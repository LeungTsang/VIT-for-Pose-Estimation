...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, num_frames = 6, img_size=(192,640), patch_size=32, in_chans=3, embed_dim=768):
        super().__init__()
        #img_size = (H, W)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, model_name='vit_large_patch16_384', num_frames = 6, img_size=(192,640), patch_size=32, in_chans=3, embed_dim=1024, depth=12,
        ...
        self.patch_embed = PatchEmbed(
                num_frames = self.num_frames, img_size=self.img_size, patch_size=self.patch_size, in_chans=self.in_chans, embed_dim=self.embed_dim)
    ...
    def forward(self, x):
        #input is a batch of video [Batch, 3, H, W]

        #currently it is for single image
        #try to extend the feature extraction from multiple frames like Timesformer 
